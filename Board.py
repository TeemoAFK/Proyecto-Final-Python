    ...
    def CreateFireball(self, location, kongIndex):
       if len(self.Fireballs) < len(self.Enemies) * 6+6:
            self.Fireballs.append(
                Bola(pygame.image.load('Assets/fireballright.png'), (location[0], location[1] + 15), len(self.Fireballs),
        ...
        for x in range(len(self.map)):
            for y in range(len(self.map[x])):
                if self.map[x][y] == 1:
                    ...
                    self.Ladders.append(OnBoard(pygame.image.load('Assets/ladder.png'), (y * 15 + 15 / 2, x * 15 + 10 / 2)))
